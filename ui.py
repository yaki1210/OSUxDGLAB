...
class OSUApp:
    ...
    def _create_mode_tab(self, tab, mode_name, waveforms):
        ...
        if mode_name == 'PP':
            widgets['threshold_var'], _, widgets['threshold_entry_var'] = self._create_linked_slider_entry(tab, "PP 下降阈值:", 3, (0.01, 50.0), 1.00)
            ...
            # 保证变量唯一性：只创建一次
            if 'combo_enabled_var' not in widgets:
                widgets['combo_enabled_var'] = ctk.BooleanVar(value=False)
            if 'pp_enabled_var' not in widgets:
            ...
            combo_settings_frame.grid_columnconfigure(1, weight=1)
            combo_settings_frame.grid(row=1, column=0, columnspan=3, sticky="ew") # 始终显示
            # PP奖励设置的容器
            pp_settings_frame = ctk.CTkFrame(tab, fg_color="transparent")
            pp_settings_frame.grid_columnconfigure(1, weight=1)
            pp_settings_frame.grid(row=2, column=0, columnspan=3, sticky="ew") # 始终显示
            # Combo 奖励设置
            ctk.CTkLabel(combo_settings_frame, text="连击奖励波形:", text_color=self.accent_color).grid(row=0, column=0, padx=10, pady=5, sticky="w")
            if 'combo_waveform_var' not in widgets:
            ...
            if 'combo_ch_a_var' not in widgets:
                widgets['combo_ch_a_var'] = ctk.BooleanVar(value=True)
            if 'combo_ch_b_var' not in widgets:
            ...
            ctk.CTkCheckBox(combo_channel_frame, text="B", variable=widgets['combo_ch_b_var'],
                            fg_color=config.ACCENT_COLOR, hover_color=config.ACCENT_COLOR_HOVER,
                            text_color=self.accent_color, checkbox_width=20, checkbox_height=20, corner_radius=config.BUTTON_CORNER_RADIUS).pack(side="left", padx=5)
            if 'combo_threshold_var' not in widgets:
                widgets['combo_threshold_var'], _, widgets['combo_threshold_entry_var'] = self._create_linked_slider_entry(combo_settings_frame, "连击阈值:", 2, (1, 1000), 100, is_int=True)
            # PP 奖励设置
            ctk.CTkLabel(pp_settings_frame, text="PP奖励波形:", text_color=self.accent_color).grid(row=0, column=0, padx=10, pady=5, sticky="w")
            if 'pp_reward_waveform_var' not in widgets:
            ...
            if 'pp_reward_ch_a_var' not in widgets:
                widgets['pp_reward_ch_a_var'] = ctk.BooleanVar(value=True)
            if 'pp_reward_ch_b_var' not in widgets:
            ...
            ctk.CTkCheckBox(pp_reward_channel_frame, text="B", variable=widgets['pp_reward_ch_b_var'],
                            fg_color=config.ACCENT_COLOR, hover_color=config.ACCENT_COLOR_HOVER,
                            text_color=self.accent_color, checkbox_width=20, checkbox_height=20, corner_radius=config.BUTTON_CORNER_RADIUS).pack(side="left", padx=5)
            if 'pp_increase_var' not in widgets:
                widgets['pp_increase_var'], _, widgets['pp_increase_entry_var'] = self._create_linked_slider_entry(pp_settings_frame, "PP增加阈值:", 2, (0.01, 100.0), 10.00)
        return widgets
    ...
    def _create_linked_slider_entry(self, parent, label, row, range_tuple, default_val, is_int=False):
        ...
        entry_var = ctk.StringVar(value=f"{int(default_val)}" if is_int else f"{default_val:.2f}")
        ...
        def update_slider(event=None):
            try:
                val = int(entry_var.get()) if is_int else float(entry_var.get())
                if range_tuple[0] <= val <= range_tuple[1]:
                    var.set(val)
                else:
                    update_entry(var.get()) # Revert if out of bounds
            except ValueError:
                update_entry(var.get()) # Revert if invalid input
        ...
        entry = ctk.CTkEntry(parent, textvariable=entry_var, width=70,
                             fg_color=config.ACCENT_COLOR, text_color=self.accent_color,
                             border_color=self.accent_color, corner_radius=config.BUTTON_CORNER_RADIUS) # 圆角
        ...
        # 双向同步
        var.trace_add('write', lambda *args: update_entry(var.get()))
        entry_var.trace_add('write', lambda *args: update_slider())
        return var, slider, entry_var

    # --- Public Methods (called by Controller) ---
    def get_current_settings(self):
        def get_channels(widgets_dict, prefix=''): # 恢复 prefix 参数，并设置默认值
            # print('[调试] get_channels prefix:', prefix, 'keys:', list(widgets_dict.keys()))
            channels = []
            ch_a_key = f'{prefix}_ch_a_var' if prefix else 'ch_a_var'
            ch_b_key = f'{prefix}_ch_b_var' if prefix else 'ch_b_var'
            # print('[调试] ch_a_key:', ch_a_key, 'val:', widgets_dict.get(ch_a_key))
            # print('[调试] ch_b_key:', ch_b_key, 'val:', widgets_dict.get(ch_b_key))
            ...
            return channels
        # print('[调试] reward_widgets keys:', list(self.reward_widgets.keys()))
        settings = {
            ...
            'reward_combo_channels': get_channels(self.reward_widgets, 'combo'), # 修正prefix
            'reward_pp_waveform': self.reward_widgets['pp_reward_waveform_var'].get(),
            'reward_pp_channels': get_channels(self.reward_widgets, 'pp_reward'), # 修正prefix
        }
    ...
    def _load_settings_on_startup(self):
        """启动时优先加载用户设置，失败则加载默认设置"""
        user_path = os.path.join(os.path.dirname(__file__), "settings", "user_settings.json")
        default_path = os.path.join(os.path.dirname(__file__), "settings", "default_settings.json")
        data = None
        if os.path.exists(user_path):
            try:
                with open(user_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                self.add_log_entry("已自动加载用户设置。");
            except Exception as e:
                self.add_log_entry(f"加载用户设置失败: {e}")
        if data is None and os.path.exists(default_path):
            try:
                with open(default_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                self.add_log_entry("已加载默认设置。")
            except Exception as e:
                self.add_log_entry(f"加载默认设置失败: {e}")
        if data is not None:
            self._apply_settings(data)
        else:
            self.add_log_entry("未找到任何设置文件，使用内置默认设置。")
            self._apply_settings(self._get_builtin_default_settings())

    def _get_builtin_default_settings(self):
        return {
            ...
            'pp_channels': ['A'],
            'pp_threshold': 1.0,
            'combo_mode_enabled': False,
            'combo_threshold': 100,
            'pp_increase_threshold': 10.0,
            'reward_combo_waveform': '潮汐',
            'reward_combo_channels': [],
            'reward_pp_waveform': '呼吸',
            'reward_pp_channels': [],
            'test_waveform': '呼吸',
            'pp_reward_enabled': False
        }

    def _apply_settings(self, data):
        # Miss
        try:
            self.miss_widgets['enabled_var'].set(data.get('miss_mode_enabled', True))
            self.miss_widgets['waveform_var'].set(data.get('miss_waveform', '潮汐'))
            self.miss_widgets['ch_a_var'].set('A' in data.get('miss_channels', ['A']))
            self.miss_widgets['ch_b_var'].set('B' in data.get('miss_channels', []))
            self.pp_widgets['enabled_var'].set(data.get('pp_mode_enabled', False))
            self.pp_widgets['waveform_var'].set(data.get('pp_waveform', '潮汐'))
            self.pp_widgets['ch_a_var'].set('A' in data.get('pp_channels', ['A']))
            self.pp_widgets['ch_b_var'].set('B' in data.get('pp_channels', []))
            self.pp_widgets['threshold_var'].set(data.get('pp_threshold', 1.0))
            # Reward
            self.reward_widgets['combo_enabled_var'].set(data.get('combo_mode_enabled', False))
            self.reward_widgets['pp_enabled_var'].set(data.get('pp_reward_enabled', False))
            if 'combo_threshold_var' in self.reward_widgets:
                self.reward_widgets['combo_threshold_var'].set(data.get('combo_threshold', 100))
            if 'combo_threshold_entry_var' in self.reward_widgets:
                self.reward_widgets['combo_threshold_entry_var'].set(str(data.get('combo_threshold', 100)))
            if 'pp_increase_var' in self.reward_widgets:
                self.reward_widgets['pp_increase_var'].set(data.get('pp_increase_threshold', 10.0))
            if 'pp_increase_entry_var' in self.reward_widgets:
                self.reward_widgets['pp_increase_entry_var'].set(str(data.get('pp_increase_threshold', 10.0)))
            self.reward_widgets['combo_waveform_var'].set(data.get('reward_combo_waveform', '潮汐'))
            self.reward_widgets['combo_ch_a_var'].set('A' in data.get('reward_combo_channels', ['A']))
            self.reward_widgets['combo_ch_b_var'].set('B' in data.get('reward_combo_channels', []))
            self.reward_widgets['pp_reward_waveform_var'].set(data.get('reward_pp_waveform', '呼吸'))
            self.reward_widgets['pp_reward_ch_a_var'].set('A' in data.get('reward_pp_channels', ['A']))
            self.reward_widgets['pp_reward_ch_b_var'].set('B' in data.get('reward_pp_channels', []))
            self.test_waveform_var.set(data.get('test_waveform', '呼吸'))
        except Exception as e:
            self.add_log_entry(f"应用设置失败: {e}")

    def _save_settings(self):
        """延迟保存当前设置到user_settings.json，3秒内多次变更只保存一次，并打印测试信息"""
        if self._save_timer:
            self._save_timer.cancel()
        # 打印当前奖励模式通道设置
        settings = self.get_current_settings()
        print("[测试] 当前reward_combo_channels:", settings.get('reward_combo_channels'))
        print("[测试] 当前reward_pp_channels:", settings.get('reward_pp_channels'))
        self._save_timer = threading.Timer(3.0, self._do_save_settings)
    ...
    def _do_save_settings(self):
        try:
            settings_dir = os.path.join(os.path.dirname(__file__), "settings")
            os.makedirs(settings_dir, exist_ok=True)
            filepath = os.path.join(settings_dir, "user_settings.json")
            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump(self.get_current_settings(), f, ensure_ascii=False, indent=4)
            print(f"[测试] 设置已保存到 {filepath}")
        except Exception as e:
            print(f"[测试] 自动保存设置失败: {e}")
            self.add_log_entry(f"自动保存设置失败: {e}")

    def _setup_auto_save(self):
        # 绑定所有重要变量的trace，变更时自动保存
        def bind_var(var):
            var.trace_add('write', lambda *args: self._save_settings())
        # Miss
        bind_var(self.miss_widgets['enabled_var'])
        bind_var(self.miss_widgets['waveform_var'])
        bind_var(self.miss_widgets['ch_a_var'])
        bind_var(self.miss_widgets['ch_b_var'])
        # PP
        bind_var(self.pp_widgets['enabled_var'])
        bind_var(self.pp_widgets['waveform_var'])
        bind_var(self.pp_widgets['ch_a_var'])
        bind_var(self.pp_widgets['ch_b_var'])
        bind_var(self.pp_widgets['threshold_var'])
        # Reward
        bind_var(self.reward_widgets['combo_enabled_var'])
        bind_var(self.reward_widgets['pp_enabled_var'])
        bind_var(self.reward_widgets['combo_threshold_var'])
        bind_var(self.reward_widgets['pp_increase_var'])
        bind_var(self.reward_widgets['combo_waveform_var'])
        bind_var(self.reward_widgets['combo_ch_a_var'])
        bind_var(self.reward_widgets['combo_ch_b_var'])
        bind_var(self.reward_widgets['pp_reward_waveform_var'])
        bind_var(self.reward_widgets['pp_reward_ch_a_var'])
        bind_var(self.reward_widgets['pp_reward_ch_b_var'])
        bind_var(self.test_waveform_var)