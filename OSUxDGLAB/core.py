...
class AppController:
    ...
    def check_triggers(self, pp_str, combo_str, accuracy_str=None, current_miss_count=0):
        ...
        # reward mode       
        if settings['combo_mode_enabled'] and current_combo is not None and current_combo > self.last_reward_combo and \
           current_combo >= self.last_reward_combo + settings['combo_threshold']:
            
            self.log_event(f"触发: 连击奖励 (连击达到 {current_combo})")
            ...
            self.last_reward_combo = current_combo
        
        if settings['pp_reward_enabled'] and current_pp is not None and current_pp > self.last_reward_pp and \
           current_pp >= self.last_reward_pp + settings['pp_increase_threshold']:
            
            self.log_event(f"触发: PP 奖励 (PP 增加至 {current_pp:.2f})")
            ...
            self.last_reward_pp = current_pp

        # if current_combo == 0 and self.prev_combo is not None and self.prev_combo > 0:
        #     self.log_event("检测到地图重置，重置奖励基线。")
        #     self.last_reward_combo = 0
        #     self.last_reward_pp = current_pp if current_pp is not None else 0.0
